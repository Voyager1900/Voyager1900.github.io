...
// the skip value
// the number of seconds to skip forward or backward
// 10s is widely applied by many media players
let skipValue = 10;
...
videoElement.addEventListener('timeupdate', () => {
  ...
  // I'd like to have the currentime/totaltime showed, to increase user accessiblity. 
  // I asked chatgpt how I should do it, it's not working initially but I tried to debug
...
progressBar.addEventListener('input', () => {
  const time = (progressBar.value * videoElement.duration) / 100;
...
/*
I want to adjust the audio off and on
...
function muteUnmute(){
  //check if working
  //console.log("mute/unmure is working");
...
/* rewind/fastforward */
//rewind fastforward function can be very useful when user want to go back to the scene several seconds ago

function rewind() {
  videoElement.currentTime -= skipValue;
}
...
videoElement.addEventListener('error', function() {
  console.log('Video error: ', videoElement.error.code);
  //Provide feedback to the user when the video failed
  alert('Sorry, an error occurred while loading the video.');
});
